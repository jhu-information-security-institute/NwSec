...
1. `$ make -j4 V=s`
    ...
    * Note: build takes around X minutes 5:25 when running from a container that is inside a VM
...
1. create bootable, live debian image (download from teklager here, https://teklager.se/static/images/debapu-live-2023-11-26.iso)
    * `$ sudo dd if=debapu-live-2023-11-26.iso of=/dev/sdc bs=1M status=progress`
...
1. boot apu4 with USB drive containing bootable, live debian image (also see here, https://teklager.se/en/knowledge-base/openwrt-installation-instructions/)
