...
int main(int argc, char **argv) {
	...
	while((option=getopt_long(argc, argv, "c:s:t:nhr:f:d:D:l:L:p:P:o:O:z:q:vCS", longopts, NULL)) != -1) {
		switch(option) {
			case 'c':	/* Client data */
			...
			case 'h':	/* help */
				puts( "icmp-quench\nAn ICMP-based throughput-reduction attack tool\n");
				usage();
				puts(	"\nOPTIONS:\n"
	...
	if(getuid()) {
		puts("icmp-quench needs root privileges to run.");
		exit(1);
	}
...
void usage(void) {
	/* TODO: actualizar */
	puts("usage: icmp-quench -c CLIENT_IP[:PORT] -s SERVER_IP[:PORT] [-t client|server] [-n | -f SOURCE_ADDRESS] [-r RATE | -d DELAY] [-D PAUSE] [-l IP_TTL] [-L IPP_TTL] [-o IP_TOS] [-O IPP_TOS] [-z IP_SIZE] [-q TCP_SEQ]");
}
